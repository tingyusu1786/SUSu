...
const BrandCatalogue: React.FC<CatalogueProps> = ({
  ...
  if (!allBrandsInfo[pageBrandId]) {
    return (
      <div className='mx-auto mt-5 max-w-[960px] text-center text-3xl'>
        brand not found ☹
      </div>
    );
  }
  return (
    <div className='mx-auto mt-5 max-w-[960px] '>
      <div className='flex items-center gap-5'>
        <h1 className='inline-block text-6xl selection:bg-green-400 sm:text-5xl'>
          {allBrandsInfo[pageBrandId].name}
        </h1>
        <img
      ...
      <div className='mb-5 flex items-center justify-start gap-1 text-lg'>
        ...
        />
        <span className='mr-5'>
          {allBrandsInfo[pageBrandId].numStore || '?'}
        </span>
              ...
              <div className='flex flex-wrap content-start gap-3'>
                {itemsOfCategory.length !== 0 &&
                  itemsOfCategory.map((item: string[]) => (
                    <Link
                      to={`/drinkipedia/${allBrandsInfo[pageBrandId].brandId}/${item[0]}`}
                      key={item[0]}
                    >
                      <div
                        key={item[0]}
      ...
      <div className='mb-5 text-center text-sm'>
        (private logs may not be shown)
      </div>
      <PostsFeed
        currentPage='brand'
        pageBrandId={allBrandsInfo[pageBrandId].brandId}
      />
    </div>
  );
};
